...
selection <- function(n, x, sd, s, m, strategy){
  ...
  for (i in 1:10^3){
    ...
    # selection distribution based on strategy (for some reason i can't get the if else statement to work i'm not sure why) // still doesn't seem to work
    repro <- ifelse(strategy, moran(s,fitness), moran.inverse(s,fitness))
    #if (strategy == 1){
    #  repro <- moran(s,fitness)
    #} else{
    #  repro <- moran.inverse(s,fitness)
    #}
    # AB: try giving strategy as logical (TRUE/FALSE) instead and can use ifelse...
    #       ifelse(strategy, moran(s, fitness), moran.inverse(s, fitness))
...
```{r}
test <- selection(length(x), x, 1-x, 1, 0.05, TRUE)
test2 <- selection(length(x), x, 1-x, 0, 0.05, FALSE)
